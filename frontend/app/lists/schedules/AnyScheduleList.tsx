...
import {AddAnyScheduleModal} from "~/modals/AddItemModal/schedules/AddAnyScheduleModal";
import {useAnySchedulePostFormContext} from "~/forms/schedule/contexts";
import {ANY_SCHEDULE_PARAMS} from "~/page_params/schedules";

const NAME_COLUMN: Column<AnyScheduleGet> = {
    heading: "Name",
    hasLink: true,
...
const DESCRIPTION_COLUMN: Column<AnyScheduleGet> = {
    heading: "Description",
    hasLink: false,
...
const DEFAULT_SORT_BY: SortBy<AnyScheduleGet>[] = [{
    column: NAME_COLUMN,
    reversed: false,
}, {
...
export function AnyScheduleList(
    ...
    const onAdd = useCallback(() => {
        const initialValues: AnySchedulePost = {
            type: "any_schedule",
            name: "",
            description: "",
            schedules: [],
    ...
    const onDelete = useCallback((item: AnyScheduleGet) => {
        setToDelete(item)
        setConfirmDeletePrompt(ANY_SCHEDULE_PARAMS.confirmDeleteItemModalPrompt(item))
    ...
    return <>
        <Modal.Stack>
            ...
            <ConfirmDeleteModal
                ...
                title={ANY_SCHEDULE_PARAMS.confirmDeleteItemModalTitle}
