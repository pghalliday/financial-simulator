...
from financial_simulator.actions import TickAction, Action
...
class Engine(Iterator[Tuple[date, Sequence[Entity]]]):
    ...
    def __dispatch(self, action: Action) -> Iterator[Action]:
        self.current_entities, action_sequences = zip(*(entity.dispatch(action)
                                                        for entity
                                                        in self.current_entities))
        return (action
                for action_sequence
                in action_sequences
                for action
