...
class Account:
    ...
    def __propagate_change(self, sub_account_name: str, change: Change) -> Self:
        return replace(self, sub_accounts=tuple(sub_account.__enter_change(change)
    ...
    def open_year(self) -> Sequence[Change]:
        change = Change(amount=self.balance,
                        account_path=())
        sub_accounts_and_changes = tuple(
            (sub_account.name, sub_account.open_year()) for sub_account in self.sub_accounts)
        return (change,) + tuple(replace(change,
