...
class AllSchedule(Schedule):
    ...
    def check(self, current_date: date) -> Tuple[Self, bool] | None:
        if not self.schedules:
            return None
        schedules_and_scheduled = tuple(schedule.check(current_date) for schedule in self.schedules)
        if any(schedule_and_scheduled is None for schedule_and_scheduled in schedules_and_scheduled):
            return None
