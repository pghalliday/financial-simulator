...
class List(dmc.Box):
    class ids:
        ...
        location_id_store = list_aio_id_creator("location-id-store")
    ...
    def __init__(
        ...
        super().__init__(
            [
                ...
                dcc.Store(id=self.ids.location_id_store(aio_id), data=location_id),
    ...
    def complete_delete(delete_action_raw: Dict[str, Any], items: Sequence[ListItem]) -> Patch:
        patch = Patch()
    ...
    @staticmethod
    @callback(
        Output(ConfirmDeleteItemPopup.ids.action_store(MATCH), "data", allow_duplicate=True),
