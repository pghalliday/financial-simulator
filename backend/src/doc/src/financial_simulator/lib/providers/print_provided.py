...
def generate_provider_results(
    initial_date: date, initial_provider: Provider[T]
) -> Generator[Tuple[date, Sequence[T] | None]]:
...
def print_provided(
    initial_date: date, initial_provider: Provider[T], number_of_days: int
) -> None:
    results = tuple(
        islice(
            generate_provider_results(initial_date, initial_provider), number_of_days
        )
    )
