...
from financial_simulator.lib.providers import AlwaysProvider
from financial_simulator.lib.rates import ContinuousRate
from financial_simulator.lib.schedules import DailySchedule
...
for year in YEARS:
    ...
    balance_change = (
        bank_account.closing_balance[year] - bank_account.opening_balance[year]
    )
    print(f"Balance change for year {year}: {balance_change:.6f}")
    print()
    print(
        f"Opening interest applied for year {year}: {bank_account.opening_interest_applied[year]:.6f}"
    )
    print(
        f"Closing interest applied for year {year}: {bank_account.closing_interest_applied[year]:.6f}"
    )
    interest_applied_change = (
        bank_account.closing_interest_applied[year]
        - bank_account.opening_interest_applied[year]
    )
    print(f"Interest applied change for year {year}: {interest_applied_change:.6f}")
    print()
    print(
        f"Opening interest accrued for year {year}: {bank_account.opening_interest_accrued[year]:.6f}"
    )
    print(
        f"Closing interest accrued for year {year}: {bank_account.closing_interest_accrued[year]:.6f}"
    )
    interest_accrued_change = (
        bank_account.closing_interest_accrued[year]
        - bank_account.opening_interest_accrued[year]
    )
